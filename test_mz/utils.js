...
exports.deployCrossLibrary = async function (deployer) {
  const CrossLib = await ethers.getContractFactory("CrossLibrary", {
    signer: deployer,
  });
  const crossLib = await CrossLib.deploy();
  await crossLib.deployed();
...
exports.deployRepay = async function (deployer, crssAddr, rSyrupAddr, crssPerBlock, startBlock, wireLib) {
  ...
  const repay = await Repay.connect(deployer).deploy(crssAddr, rSyrupAddr, crssPerBlock, startBlock);
