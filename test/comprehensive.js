...
let wireLib, factory, wbnb, center, crossLib, routerLib, maker, taker, crss, mock, mock2, farm, farmLib, xCrss, referral, rCrss, rSyrup, repay;
...
describe("====================== Stage 3: Test Dex functionalities. ======================\n".yellow,
  async function () {
    ...
    it("Add liquidity and remove liquidity were checked.\n".green, async function () {
      ...
      await crss.connect(owner).transfer(alice.address, ethToWei(15000));
      ...
      [report, crssAmount] = await test_swap(wbnb, 1, crss, undefined, alice, bob, true);
      [report, bnbAmount] = await test_swap(crss, crssAmount * 0.99, wbnb, undefined, bob, alice, true);
      [report, crssAmount] = await test_swap(crss, undefined, wbnb, bnbAmount * 0.99, alice, bob, true); //----------------------------
      ...
      [report, crssAmount] = await test_swap(mock2, undefined, crss, 100, alice, bob, true); //----------------------------------
...
describe("=========================== Compensation Test =======================\n".yellow, async function () {
  ...
  it("Compensation test.\n".green, async function () {
    ...
    (await tx).wait();
    console.log("\tAlice could harvest repay after the farm is resumed".yellow);
...
async function showUserInfo(pid, _user) {
  ...
  let blockNo, pendingCrss, pendingPerBlock, collectiveCrss, collectiveCompound,  rewardPayroll, collectOption, deposit, accRewards, 
  totalVest, totalMatureVest, lpBalance, crssBalance, totalAccRewards;

  [ [blockNo, pendingCrss, pendingPerBlock, collectiveCrss, collectiveCompound,  rewardPayroll], 
    [collectOption, deposit, accRewards, totalVest, totalMatureVest, lpBalance, crssBalance, totalAccRewards]
  ...
  return {
    symbol: symbol, blockNo: blockNo, deposit: deposit, accReward: accRewards, tVest: totalVest, wVest: totalMatureVest, pending: pendingCrss, pendingPerBlock: pendingPerBlock,
    collectiveCrss: collectiveCrss, collectiveCompound: collectiveCompound, payroll: rewardPayroll, lp: lpBalance, crss: crssBalance, tAccRewards: totalAccRewards
  };
}
...
const RewardOptions = ["FullProcess", "IndividualOnly", "NoProcess" ];
...
async function deposit(pid, caller, amount, revert) {
  ...
  let behavior;
  [ behavior, ] = await farm.getUserState(pid, caller.address);
  ...
  (await farm.connect(caller).deposit(pid, ethToWei(amount), behavior)).wait(); // ----- UI devs, pass 'behavior' as the last argument.
...
async function withdraw(pid, caller, amount) {
  ...
  console.log(`\tWithdrawing now... taking pending rewards first.`.green);
  (await farm.connect(caller).withdraw(pid, ethToWei(amount), behavior)).wait();
...
async function withdrawVest(pid, caller, amount) {
  ...
  (await pair.connect(caller).approve(farm.address, ethToWei(amount))).wait();
  (await farm.connect(caller).withdrawVest(pid, ethToWei(amount), behavior)).wait();
...
async function vestAccumulated(pid, caller) {
  ...
  (await farm.connect(caller).vestAccumulated(pid, behavior)).wait();
...
async function compoundAccumulated(pid, caller) {
  ...
  (await farm.connect(caller).compoundAccumulated(pid, behavior)).wait();
...
async function harvestAccumulated(pid, caller) {
  ...
  (await farm.connect(caller).harvestAccumulated(pid, behavior)).wait();
...
async function stakeAccumulated(pid, caller) {
  ...
  tx = farm.connect(caller).stakeAccumulated(pid, behavior);
  (await tx).wait();
...
async function emergencyWithdraw(pid, caller) {
  ...
  (await farm.connect(caller).emergencyWithdraw(pid, behavior)).wait();
...
async function switchCollectOption(pid, caller, newOption) {
  ...
  tx = (await farm.connect(caller).switchCollectOption(pid, CollectOptions.indexOf(newOption), behavior)).wait();
...
async function withdrawCommission(caller, amount) {
  console.log("\t%s is going to withdraw referral commission of %s %s".yellow, caller.name, amount, "Crss");

  let crss0, crss1;
  ret = await showUserInfo(0, caller);
  crss0 = ret.crss;

  let outstanding0 = await farm.getOutstandingCommission(caller.address);
  console.log("\t%s has outstanding commission: ", caller.address, outstanding0);

  console.log(`\tWithdrawing referral commission now...`.green);
  (await farm.connect(caller).withdrawOutstandingCommission(ethToWei(amount))).wait();

  let outstanding1 = await farm.getOutstandingCommission(caller.address);
  console.log("\t%s has outstanding commission: ", caller.address, outstanding1);

  ret = await showUserInfo(0, caller);
  crss1 = ret.crss;

  console.log(`\t%s's Crss balance increased %s %s`.cyan, caller.name, crss1 - crss0, "Crss");
  console.log(`\t%s's Crss outstanding commission decreased %s %s`.cyan, caller.name, outstanding0 - outstanding1, "Crss");
}
...
describe("====================== Stage 5: Test CrossFarm ======================\n".yellow,
  function () {
    ...
    it("How the initial empty Crss staking pool works with rewards.\n".green, async function () {
      ...
      console.log("\tTransferring...");
      await crss.transfer(alice.address, ethToWei(amount * 1.05));
      console.log("\tTransferring...");
      await crss.transfer(bob.address, ethToWei(amount * 1.05));
      ...
      await withdraw(0, bob, 900);
      await withdraw(0, carol, 900);
    ...
    it("How the initial empty Crss/Bnb staking pool works with rewards.\n".green, async function () {
      tx = farm.connect(owner).add(80, crss_bnb.address, true, 2000);
      (await tx).wait();
...
//        await deposit(0, alice, 10);  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    it("Alice, Bob, and Carol each were minted Mock tokens.\n".green, async function () {
      tx = farm.connect(owner).add(50, crss_mck.address, true, 1000);
      (await tx).wait();
    ...
    it("Round 1. Alice, Bob, and Carol swapped a big amount of Bnb for Crss, and staked the Crss tokens.\n".green, async function () {
      ...
      [report, crssAmount] = await test_swap(wbnb, undefined, crss, 100, alice, alice, true);
      await deposit(1, alice, weiToEth(await crss_bnb.balanceOf(alice.address))/10 );

      [report, crssAmount] = await test_swap(mock, 100, crss, undefined, alice, alice, true);
      ...
      [report, crssAmount] = await test_swap(mock, 1000, crss, undefined, bob, bob, true);
      ...
      [report, crssAmount] = await test_swap(mock, 2000, crss, undefined, carol, carol, true);
...
//        await deposit(0, alice, 10);  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    it("Add Crss_Mock2 pool.\n".green, async function () {
      tx = farm.connect(owner).add(80, crss_mck2.address, true, 1000);
      (await tx).wait();
    ...
    it("Continue after replacing Farm.\n".green, async function () {
      ...
      await deposit(1, alice, 10);
      // console.log("\tBob Balance: %s", await crss_bnb.balanceOf(alice.address), await crss_bnb.balanceOf(bob.address))
    ...
    it("Test Referral", async () => {
      tx = farm.connect(owner).changeReferrer(alice.address, bob.address);
      expectReverted(tx);
      console.log("\tOwner couldn't change alice's referrer, as designed");

      tx = farm.connect(carol).changeReferrer(alice.address, bob.address);
      (await tx).wait();
      console.log("\tCarol the backend caller changeed alice's referrer");

      let outstanding0 = await farm.getOutstandingCommission(bob.address);
      console.log("\tBob the referrer of Alice has outstanding commission: ", outstanding0);
      await deposit(1, alice, 100);
      let outstanding1 = await farm.getOutstandingCommission(bob.address);
      console.log("\tBob the referrer of Alice has outstanding commission increased by: %s", outstanding1 - outstanding0);

      outstanding0 = await farm.getOutstandingCommission(alice.address);
      console.log("\tAlice the referrer of no one has outstanding commission: %s", outstanding0);

      

      await withdrawCommission(alice, 100);
      await withdrawCommission(bob, 100);
    })
    ...
    it("Test Enlist and Delist.\n".green, async function () {
      ...
      //tx = farm.connect(alice).deposit(1, ethToWei(10), 1, 0);
